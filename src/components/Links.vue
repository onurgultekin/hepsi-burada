<template>
  <div class="max-w-lg rounded overflow-hidden shadow-md mx-auto my-6">
    <div v-if="links.length > 0" class="px-6 py-4">
      <select v-model="sortBy" class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
    ...
    <div v-else class="bg-red-100 border-t-4 border-red-500 rounded-b text-red-900 px-4 py-3 shadow-md" role="alert">
      <div class="flex">
        ...
        <div>
          ...
          <p class="text-sm">You can add new one by clicking on submit a link button above.</p>
        </div>
      </div>
    </div>
  </div>
